...
export default function FieldSettings() {
    return <>
        ...
        <p style={{ display: "inline-block", fontSize: "max(min(1.7vw, 15px), 10px)" }}>inverse y</p>
        <br />
        <label className="option_text">Background image: </label>
        <input type="file" id="img_input" name="img_input" accept="image/*" onChange={set_background_img}></input>
        <button id="clear_img" style={{ width: "auto"}} onClick={clear_background_img}>clear image</button>
        <br />
